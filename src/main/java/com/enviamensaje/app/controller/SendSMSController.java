...
public class SendSMSController {
	...
	public String sendsms(@RequestParam(value = "phone", required = true) String phone,
			@RequestParam(value = "text", required = true) String text, final ModelMap model)
	...
	private void send(final String phone, final String text) throws InvalidPhoneException {
		if (isPhoneValid(phone)) {
			final TwilioRestClient client = new TwilioRestClient.Builder(smsConfiguration.getSid(),
					smsConfiguration.getToken()).build();
